...
// Función que se ejecutará en cada interrupción del Ticker
void toggleLED() {
...
public:
    ...
    {
        static const char name[] = "ZAPATILLA IOT";
        return name;
    }
...
protected:
    ...
    void onConnectionComplete(const ble::ConnectionCompleteEvent &event) override
    {
        if (event.getStatus() == BLE_ERROR_NONE) {
            ...
            ticker.detach(); // PARAMOS EL PARPADEO LED LED
            LEDBT = 0;
            printf("Connected to: ");
    ...
    {
        printf("Disconnected.\r\n");
        ticker.attach(&toggleLED, std::chrono::seconds(1) ); // Volvemos a encender el LED 
        start_activity();
    }
    ...
    {
        ticker.attach(&toggleLED, std::chrono::seconds(1) );
        ble_error_t error;
